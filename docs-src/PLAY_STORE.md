...
تبلیغ در اپ از قبل پیاده شده: **فقط یک تبلیغ تمام‌صفحه، یک بار در هر اجرا، درست بعد از ورود** (بدون بنر، بدون تبلیغ وسط کار). زمان دکمهٔ رد/بستن داخل تبلیغ را خود تپسل تعیین می‌کند (معمولاً چند ثانیه؛ برای ویدیوهای طولانی‌تر نوع جایگاه «ویدیویی جایزه‌دار» را در پنل انتخاب کن). تا وقتی شناسه‌های واقعی را ندهی، **تبلیغ‌های تستی تپسل** نمایش داده می‌شود (درآمدی ندارد).
...
5. در GitHub → Settings → Secrets → Actions، ۲ تا secret بساز:
